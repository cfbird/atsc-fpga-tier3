...
//   3. Resets with 0x018F at the start of each Data Field (every 58,344 bytes).
// ============================================================================
...
module atsc_derandomizer (
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // Track 58,344 bytes per field (312 segments * 187 bytes)
            if (byte_in_field_cnt >= 16'd58343) begin
                byte_in_field_cnt <= 16'd0;
                lfsr <= 14'h018F;
            end else begin
                byte_in_field_cnt <= byte_in_field_cnt + 16'd1;
            end
        end else begin
            out_byte_valid <= 1'b0;
        end
